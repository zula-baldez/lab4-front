...
function Graph() {
    ...
    useEffect(() => {
        ...
        canvas.onmousedown = (e) => {
            ...
            sendAttempt(x, y, r, dispatch, navigate)
        }
        drawGraph()
    }, [dispatch, table, r])
    return (
...
//-1 means last page
export async function sendAttempt(x, y, r, dispatcher, navigate) {
    ...
    await fetch(BASE_URL + '/attempt', requestOptions)
        .then(async result => {
                if (await result.status !== 200) {
                    if (refresher() === true) {
                        await sendAttempt(x, y, r, dispatcher, navigate)
                    }
                }
            }
        ).catch(e => {
    ...
    dispatcher(fetchDate(-1, 10, navigate))
}
